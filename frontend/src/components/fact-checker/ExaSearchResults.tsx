...
import type { ExaSearchResponse, ExaSearchResult } from '../../types';
...
export const ExaSearchResults: React.FC<ExaSearchResultsProps> = ({
  result,
  isDarkMode
}) => {
  return (
        ...
        <div className="flex items-center">
          ...
          <div className="ml-4">
            ...
            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              ...
              {result.searchType && ` (${result.searchType} search)`}
              {result.costDollars && ` • Cost: $${result.costDollars.toFixed(4)}`}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6">
        <div className="mb-6">
          ...
          <ul className="space-y-4">
            {result.results.map((result: ExaSearchResult, index: number) => (
